...
parsnip::fit(my_big_tuna, data = heart_data_tr) %>% predict(heart_data_tr) %>% rmse(estimate = .[[1]], truth = heart_data_tr$heart_disease)
```
...
hd_recipe %<>% 
  ...
  add_role(sex, exercise_angina, high_sugar, chest_pain, slope, new_role = 'sneaky_dummies')
```
...
```{r, step 1}
hd_recipe %<>% step_mutate_at(has_role('sneaky_dummies'), -all_outcomes(), fn = as.factor)
```
...
hd_recipe %<>% 
  update_role(id, new_role = 'id_variables') %>% 
  add_role(sex, high_sugar, exercise_angina, chest_pain, vessels, slope, new_role = 'sneaky_dummies')
```
...
hd_recipe %<>% 
  update_role(id, new_role = 'id_variables') %>% 
  add_role(heart_disease, sex, high_sugar, exercise_angina, chest_pain, vessels, slope, new_role = 'sneaky_dummies')
```
...
This uses the .hi[bake] functionality of recipes. 

--

By .hi-blue[prepping] the recipe, we now have a set of directions with numeric transformations that can be generalized out of sample.
